# AISO-Teams Backend — Development Setup
...
> **Note:** parts of this guide were written during Sprint 1 and may show Sprint-1-era behaviour
> (e.g. the bot "echoing" messages). The current bot returns Sales Order Adaptive Cards from mock
> SAP data. Treat the toolchain/Docker/run steps as authoritative; a full content refresh is pending.
...
> Docker dev services (`docker-compose.yml`) live in [`infrastructure/docker/`](../../infrastructure/docker/), not under `backend/`.
...
The `docker-compose.yml` file in `infrastructure/docker/` defines two services: `postgres` and `redis`.

```bash
# From infrastructure/docker/ folder
...
```
[+] Running 2/2
 ✔ Container aiso_postgres  Started
 ✔ Container aiso_redis     Started
```
...
aiso_postgres    postgres:16-alpine   Up (healthy)        0.0.0.0:5432->5432/tcp
aiso_redis       redis:7-alpine       Up                  0.0.0.0:6379->6379/tcp
```
...
|---|---|---|---|---|---|
| PostgreSQL | localhost | 5432 | `aiso_user` | `aiso_dev_password` | `aiso` |
...
```bash
# Test PostgreSQL
docker exec -it aiso_postgres psql -U aiso_user -d aiso -c "SELECT version();"
# Should print PostgreSQL version

# Test Redis
docker exec -it aiso_redis redis-cli ping
# Should print: PONG
```
...
# 3. Make sure containers are running (from repo root)
...
See [Git Workflow](git-workflow.md) for branch naming and commit message rules. See [Naming Convention](AISO_Naming_Convention.docx) for code style.
...
> Run these from `infrastructure/docker/` (where `docker-compose.yml` lives), or add `-f infrastructure/docker/docker-compose.yml` from the repo root.
...
docker exec -it aiso_postgres bash      # Shell into a container
```
...
- Architectural questions? See [Technical Specification](../03-specifications/technical-specification.md) (when available).
