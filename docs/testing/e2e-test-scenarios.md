...
| # | Input | Expected |
|---|---|---|
| 6.1 | `forward order 13122` (no recipient) | **Confirm** card; recipients filtered by SO **SalesOrg** (VKORG) |
| 6.1b | Confirm → pick recipient → **Send** | Success forwarded; recipient = SAP User ID |
| 6.2 | `forward order 13122 to DEV-024` | Confirm card; DEV-024 pre-selected when linked + same org (or unscoped) |
| 6.3 | Cancel confirm | No forward |
| 6.4 | Display names on picker | Title shows `Name (DEV-xxx)`; value = SAP User ID; other-org managers hidden when same-org peers exist |
